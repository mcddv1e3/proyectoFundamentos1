...
def refrescar(value,value1,vurl1,imagen_mascara): 
    ...
    if value == "COVID":
        ...
        covid2.adicionar(imagen2,imagenm2,vurl1,'IMAGENESADICION/imagenes','IMAGENESADICION/mascaras','DATOS')
    if value == "Lung_Opacity":
        LungOpacity1 = LungOpacity.LungOpacity('NombreProvisional.png',directorioR)
        LungOpacity1.adicionar(imagen2,imagenm2,vurl1,'IMAGENESADICION/imagenes','IMAGENESADICION/mascaras','DATOS')
    if value == "Normal":
        Normal1 = Normal.Normal('NombreProvisional.png',directorioR)
        Normal1.adicionar(imagen2,imagenm2,vurl1,'IMAGENESADICION/imagenes','IMAGENESADICION/mascaras','DATOS')    
    if value == "Viral Pneumonia":
        ViralPneumonia1 = ViralPneumonia.ViralPneumonia('NombreProvisional.png',directorioR)
        ViralPneumonia1.adicionar(imagen2,imagenm2,vurl1,'IMAGENESADICION/imagenes','IMAGENESADICION/mascaras','DATOS')    

        
i = 0
btnEnv = Button(root,text="Escoja Imagen Principal",command=select_file).grid(row=i)    
i = i+1
btnEnv = Button(root,text="Escoja Imagen Mascara",command=select_file1).grid(row=i)
i = i+1
titulo = Label(root,text="Elija a cual categoria corresponden las imagenes:").grid(row=i)
...
btnEnv=Button(root,text="Guardar",command=lambda:refrescar(carpetas.get(), imagenElegida.get(), vurl1.get(),imagenElegidaMascara.get())).grid(row=i)
root.mainloop()    
